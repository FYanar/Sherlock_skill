...
<#
.SYNOPSIS
    Sherlock 3-Uzman Ajan Rol Kurulum Script'i — Evrensel Platform Desteği
.DESCRIPTION
...
try {
    ...
    foreach ($sourceFile in Get-ChildItem -LiteralPath $sourcePath -Filter '*.toml' -File) {
        # FLAT layout: ~/.codex/agents/sherlock-code-analyst.toml
    ...
    Write-Host "[OK] Sherlock 3-Uzman Ajan Rol Kurulumu Tamamlandı (ACID Verified)."
