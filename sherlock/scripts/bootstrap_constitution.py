...
def bootstrap_project(
    ...
    # 3. Graphify SSOT Hook (references/graphify-integration.md)
    graphify_adapter = os.path.join(SCRIPT_DIR, "graphify_adapter.py")
    if os.path.exists(graphify_adapter):
        try:
            print(f"  [~] Graphify SSOT hook: ensuring graphify-out/graph.json ...")
            subprocess.run([sys.executable, graphify_adapter, "--workspace", workspace_root, "--ensure", "--export-findings"],
                           timeout=310)
            print(f"  [+] Graphify context generated: .sherlock/graphify-context.md")
            print(f"  [+] Visual Reports generated: graph.html, CALLFLOW.html, pipeline_graph.html")
        except subprocess.TimeoutExpired:
            print(f"  [!] Graphify hook timed out (300s) — continuing with fallback")
        except Exception as e:
            print(f"  [!] Graphify hook note (non-blocking): {e}")

    # 3.6 .graphifyignore otomatik oluştur (hiz icin kritik)
    try:
        try:
            from graphify_adapter import _ensure_graphifyignore
        except ImportError:
            from scripts.graphify_adapter import _ensure_graphifyignore
        _ensure_graphifyignore(workspace_root)
        print(f"  [+] .graphifyignore otomatik olusturuldu (hiz iyilesmesi icin)")
    except Exception:
        pass  # non-blocking
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prune-memory", action="store_true",
                        help="Enforce Agent/memory.md 1000-line budget via memory_manager.py --prune")
    parser.add_argument("--memory-budget", type=int, default=1000,
                        help="Memory line budget (default: 1000)")
    args = parser.parse_args()

    if args.prune_memory:
        try:
            from memory_manager import prune_memory as _prune
        except ImportError:
            try:
                from scripts.memory_manager import prune_memory as _prune
            except ImportError:
                import importlib.util as _ilu
                _spec = _ilu.spec_from_file_location(
                    "sherlock_memory_manager",
                    os.path.join(SCRIPT_DIR, "memory_manager.py"))
                _mod = _ilu.module_from_spec(_spec)
                _spec.loader.exec_module(_mod)
                _prune = _mod.prune_memory
        res = _prune(args.target, budget=args.memory_budget)
        print(f"[SHERLOCK-MEMORY] Prune result: {res}")
        sys.exit(0)
